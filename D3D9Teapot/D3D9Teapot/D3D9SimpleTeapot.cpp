...
BOOL InitDevice(HWND hWnd)
{
  ...
  if (g_wire)
    ...
    g_SceneGraph.SetDevice(g_pDevice);
    g_SceneGraph.SetEffect(g_pEffect9);
...
void Render()
{
  ...
  D3DXMatrixTranslation(&mTranslate, 1.0f, 2.0f, 0.0f);
  D3DXMatrixScaling(&mScale, 0.5, 0.5, 0.5);
  ...
  D3DXMatrixPerspectiveFovLH(&mProj, (40.0f/180.0f)*D3DX_PI, 1.0f, 0.1f, 25.0f);
  //g_pDevice->SetTransform( D3DTS_PROJECTION, &mProj);
#endif

  //Set the textures
  if (FAILED( hr = g_pEffect9->SetTexture( "g_DiffuseTex", g_pDiffuseTex ) ) )
    g_result = 1;
  ...
  if (FAILED( hr = g_pEffect9->SetMatrix( "g_mWorldInverse", &mWorldInverse ) ) )
  	g_result = 1;
