...
struct VS_OUTPUT
{
		float4 vPos      	: POSITION0;	// Xfmed Wvp vertex position
...
VS_OUTPUT VS_woNorm( VS_IN In )
{
		...
		Out.vDiffuse.rgb =  ambient + diffuse;
		Out.vDiffuse.a = g_Diffuse.a;
	  Out.vSpec		= float4(spec, 0.0f);
...
float4 PS_woNorm( VS_OUTPUT In, uniform bool bTexture ) : COLOR
{
	...
	float3 diffuse	= In.vDiffuse.rgb * texColor;
	
	return float4(diffuse + In.vSpec.rgb, In.vDiffuse.a);
